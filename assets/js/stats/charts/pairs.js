/* assets/js/stats/charts/pairs.js
 *
 * Top city pairs — ranked horizontal bars, one bar per undirected city pair.
 * Bar length encodes flight count (the ranked measure); total distance rides
 * alongside as a muted secondary column.
 *
 * Data: window.FlightMetrics.topCityPairs(legs, n)
 *       -> [{ pair, a, b, flights, km }]
 *
 * Ties are first-class: the real data has a 19x tie at rank 1 (Duesseldorf <->
 * Munich and Denver <-> New York). Tied rows share a rank rendered as "=1",
 * are ordered alphabetically inside the tie group (a deliberately non-value
 * ordering), and a footnote states this so the layout never implies a winner.
 *
 * Plain global script. Registers window.StatsCharts.pairs.
 */
...
(function () {
    ...
    /* Group rows of equal `flights` into shared ranks. Inside a tie group rows
       are ordered alphabetically by label so the vertical order carries no
       value information. */
    function rank(rows) {
        var sorted = rows.slice().sort(function (a, b) { return b.flights - a.flights; });
        var out = [];
        var i = 0;
        while (i < sorted.length) {
            var j = i;
            while (j + 1 < sorted.length && sorted[j + 1].flights === sorted[i].flights) j += 1;
            var group = sorted.slice(i, j + 1).sort(function (a, b) {
                return String(a.__label).localeCompare(String(b.__label));
            });
            group.forEach(function (d) {
                out.push({ d: d, rank: i + 1, tied: group.length > 1, tieSize: group.length });
            });
            i = j + 1;
        }
        return out;
    ...
    function label(d) {
        if (d.pair) return String(d.pair);
        return [d.a, d.b].filter(Boolean).join(' ↔ ');
    }
    ...
    function draw(host, W, rows, opts) {
        ...
        var narrow = W < 540;
        var max = Math.max.apply(null, rows.map(function (d) { return d.flights || 0; }).concat([1]));
        var maxKm = Math.max.apply(null, rows.map(function (d) { return d.km || 0; }).concat([0]));
        var ranked = rank(rows);
        var anyTie = ranked.some(function (r) { return r.tied; });
        ...
        var fig = U.figure(host, 'Most-flown city pairs, ranked by number of flights');
        ...
        var H = headH + rowH * ranked.length + 4;

        var labelSize = narrow ? 12.5 : 13;
        var metaSize = narrow ? 11 : 12;

        var tipStr = U.plural(max, 'flight');
        var tipReserve = narrow ? 0 : Math.ceil(U.measure(tipStr, metaSize, 600)) + 14;
        var kmStr = U.km(maxKm);
        var kmW = narrow ? 0 : Math.ceil(U.measure(kmStr, metaSize, 400)) + 6;
        var labelW = narrow
            ? W - RANK_W
            : Math.max(110, Math.min(250, Math.round(W * 0.30)));
        var trackX = RANK_W + labelW + GAP;
        var trackW = narrow
            ? Math.max(60, W - RANK_W)
            : Math.max(60, W - trackX - tipReserve - kmW - GAP);

        var svg = U.svgRoot(fig, W, H,
            'Most-flown city pairs',
            'Horizontal bars ranked by number of flights; total distance shown alongside. ' +
            (anyTie ? 'Tied ranks are marked with an equals sign.' : ''));
        var fill = U.barGradient(svg, U.uid('pairgrad'));

        /* column headers */
        U.el(svg, 'text', {
            x: 0, y: headH - 8, fill: faintCol, 'font-size': 10.5,
        ...
        U.el(svg, 'line', {
            x1: 0, x2: W, y1: headH - 2, y2: headH - 2,
        ...
        ranked.forEach(function (r, i) {
            ...
            var g = U.el(svg, 'g');
            var full = d.__label;
            var bw = Math.max(2, trackW * (d.flights / max));

            /* rank */
            var rankTxt = (r.tied ? '=' : '') + r.rank;
            U.el(g, 'text', {
                x: 0, y: top + (narrow ? 13 : rowH / 2 + 4),
                fill: r.tied ? U.token('--accent-a') : faintCol,
                'font-size': narrow ? 11 : 12, 'font-weight': r.tied ? 600 : 400
            }, rankTxt);

            if (narrow) {
                var fitted = U.fit(full, labelW - 2, labelSize, 500);
                U.el(g, 'text', {
                    x: RANK_W, y: top + 13, fill: textCol,
                    'font-size': labelSize, 'font-weight': 500
                }, fitted.text);
                U.el(g, 'text', {
                    x: RANK_W, y: top + 29, fill: dimCol, 'font-size': metaSize
                }, U.plural(d.flights, 'flight') + ' · ' + U.km(d.km));
                grow(U.el(g, 'path', {
                    d: U.hBarPath(RANK_W, top + 34, bw, barH, 4), fill: fill
                }), i);
            } else {
                var fitted2 = U.fit(full, labelW - 2, labelSize, 500);
                U.el(g, 'text', {
                    x: RANK_W, y: top + rowH / 2 + 4.5, fill: textCol,
                    'font-size': labelSize, 'font-weight': 500
                }, fitted2.text);
                grow(U.el(g, 'path', {
            ...
            /* full-row hit target */
            var hit = U.el(g, 'rect', {
                x: 0, y: top, width: W, height: rowH, class: 'sc-hit'
            });
            U.el(hit, 'title', {}, full);
            U.hover(hit, function () {
                return '<b>' + U.esc(full) + '</b>' +
                    '<div class="sc-tip-rows">' +
                    '<span><i>Flights</i></span><span>' + U.nf(d.flights) + '</span>' +
                    '<span><i>Total distance</i></span><span>' + U.km(d.km) + '</span>' +
                    '<span><i>Per round trip</i></span><span>' + U.km(d.km / Math.max(1, d.flights)) + '</span>' +
                    '<span><i>Rank</i></span><span>' + (r.tied ? 'tied ' : '') + '#' + r.rank + '</span>' +
                    '</div>' +
                    (r.tied ? '<div style="margin-top:6px"><i>Tied with ' +
                        (r.tieSize - 1) + ' other route' + (r.tieSize > 2 ? 's' : '') +
                        ' — no ordering implied.</i></div>' : '');
            });
        });
        ...
        if (anyTie) {
            ...
            ranked.forEach(function (r) {
                if (r.tied && tiedRanks.indexOf(r.rank) < 0) tiedRanks.push(r.rank);
            });
            U.note(fig, '= marks a tied rank (' +
                tiedRanks.map(function (n) { return '#' + n; }).join(', ') +
                '). Tied routes are listed alphabetically, not by value.');
        }
    }

    window.StatsCharts.pairs = {
        render: function (container, data, opts) {
            var U = window.StatsChartUtil;
            opts = opts || {};
            var rows = (data || [])
                .filter(function (d) { return d && (d.flights > 0); })
                .map(function (d) {
            ...
            if (!rows.length) {
                ...
                var p = document.createElement('p');
                p.className = 'sc-note';
                p.textContent = 'No repeated city pairs in this selection.';
                U.ensureStyles();
                container.appendChild(p);
                return;
            }
            var limit = opts.max || 8;
            rows = rows.slice().sort(function (a, b) { return b.flights - a.flights; }).slice(0, limit);
