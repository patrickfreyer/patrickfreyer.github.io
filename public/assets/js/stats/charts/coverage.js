/* assets/js/stats/charts/coverage.js
 *
 * Country and continent coverage (~46 countries).
 *
 * Data: window.FlightMetrics.countryCoverage(legs)
 *       -> { countries: [{ country, continent, flights, km }], continents: [...] }
 *
 * FORM CHOICE — continent-clustered country chips, not a treemap:
 *   1. The payload here is WHICH countries, so the names have to be readable.
 *      46 countries with a long tail (many at 1-2 legs) means a treemap's small
 *      rectangles cannot hold their own labels — you get clipped text or a
 *      hover-only chart, which is the exact anti-pattern to avoid.
 *   2. A chip grid reflows to one or two columns at 390px. A treemap's aspect
 *      ratio does not, so its labels get worse exactly where space is tightest.
 *   3. Continent gives the nesting the brief asked for as a real grouping with
 *      its own aggregate row and share bar, rather than as an invisible layer of
 *      rectangle adjacency.
 *   4. Magnitude still reads: chip tint steps along a validated single-hue
 *      ordinal ramp AND every chip prints its leg count, so color is never the
 *      only signal.
 *
 * Degrades gracefully while `_data/heroLocations.yaml` enrichment lands: missing
 * or 'Unknown' countries/continents get a muted bucket sorted last.
 *
 * Plain global script. Registers window.StatsCharts.coverage.
 */
...
(function () {
    ...
    /* Single-hue ordinal ramp off --accent-a, validated for the dark surface:
       monotone lightness, >=0.06 adjacent dL, light end clears 2:1 vs #0a0e1a. */
    var RAMP = ['#3a4785', '#4a5aa8', '#5c6ecb', '#7688e8', '#9dabf2'];
    var UNKNOWN_BG = '#333947';
    var UNKNOWN = 'Unknown';

    function norm(v) {
        if (v === null || v === undefined) return UNKNOWN;
        var s = String(v).trim();
        if (!s || s === 'Unknown' || s === 'XXX' || s === 'null' || s === 'undefined') return UNKNOWN;
        return s;
    }
    function isUnknown(v) { return norm(v) === UNKNOWN; }

    function prepare(data) {
        var src = (data && data.countries) || [];
        var countries = src
            .filter(function (d) { return d && (d.flights > 0 || d.km > 0); })
            .map(function (d) {
                return {
                    country: norm(d.country),
                    continent: norm(d.continent),
                    flights: d.flights || 0,
                    km: d.km || 0
                };
            });

        /* continent aggregates: use the supplied array where it exists, else derive */
        var agg = {};
        countries.forEach(function (c) {
            var a = agg[c.continent] || (agg[c.continent] = {
                continent: c.continent, countries: [], flights: 0, km: 0
            });
            a.countries.push(c);
            a.flights += c.flights;
            a.km += c.km;
        });
        (data && data.continents || []).forEach(function (d) {
            if (!d) return;
            var key = norm(d.continent || d.name);
            if (!agg[key]) return;
            /* trust the metrics layer's own totals when it provides them */
            if (d.flights > 0) agg[key].flights = d.flights;
            if (d.km > 0) agg[key].km = d.km;
        });

        var continents = Object.keys(agg).map(function (k) { return agg[k]; });
        continents.forEach(function (a) {
            a.countries.sort(function (x, y) {
                if (isUnknown(x.country) !== isUnknown(y.country)) return isUnknown(x.country) ? 1 : -1;
                if (y.flights !== x.flights) return y.flights - x.flights;
                return String(x.country).localeCompare(String(y.country));
            });
        });
        continents.sort(function (x, y) {
            if (isUnknown(x.continent) !== isUnknown(y.continent)) return isUnknown(x.continent) ? 1 : -1;
            return y.km - x.km;
        });

        /* Bin country leg counts into <=5 ordinal steps.
           Quantile breaks were tried first and rejected: the distribution has a
           very heavy tail (Germany 262 / United States 214 against a long run of
           1-3 leg countries), so the top quintile lumped Germany in with Spain at
           11 legs and the ramp said nothing at the top. Fixed log-ish breaks put
           the two home countries in their own step, which is the real story.
           Breaks above the observed max are dropped, so a filtered or sparser
           dataset still gets a sensible number of steps. */
        var legs = countries.filter(function (c) { return !isUnknown(c.country); })
            .map(function (c) { return c.flights; }).sort(function (a, b) { return a - b; });
        var maxLegs = legs.length ? legs[legs.length - 1] : 0;
        var cuts = [1, 4, 11, 49].filter(function (v) { return v < maxLegs; });
        var steps = cuts.length + 1;
        var ramp = RAMP.slice(RAMP.length - steps);

        function bin(v) {
            for (var i = 0; i < cuts.length; i += 1) if (v <= cuts[i]) return i;
            return cuts.length;
        }
        countries.forEach(function (c) { c.bin = bin(c.flights); });

        /* label each non-empty bin by the values actually in it */
        var binInfo = ramp.map(function (color, i) {
            var vals = countries.filter(function (c) { return !isUnknown(c.country) && c.bin === i; })
                .map(function (c) { return c.flights; });
        ...
        return {
            ...
            ramp: ramp,
            binInfo: binInfo,
            maxContinentKm: Math.max.apply(null, continents.map(function (a) { return a.km; }).concat([1])),
            totalKm: continents.reduce(function (s, a) { return s + a.km; }, 0),
            totalFlights: continents.reduce(function (s, a) { return s + a.flights; }, 0),
            knownCountries: countries.filter(function (c) { return !isUnknown(c.country); }).length,
            knownContinents: continents.filter(function (a) { return !isUnknown(a.continent); }).length,
            unknownCountries: countries.filter(function (c) { return isUnknown(c.country); }).length
        };
    }

    function draw(host, W, model) {
        var U = window.StatsChartUtil;

        var fig = U.figure(host, 'Countries visited, grouped by continent');

        /* summary */
        var sum = document.createElement('p');
        sum.className = 'sc-cov-sum';
        sum.innerHTML = '<b>' + U.nf(model.knownCountries) + '</b> countries across <b>' +
            U.nf(model.knownContinents) + '</b> continents' +
            (model.unknownCountries
                ? ' · <b>' + U.nf(model.unknownCountries) + '</b> not yet resolved'
                : '');
        fig.appendChild(sum);

        /* ordinal scale legend */
        var scale = document.createElement('div');
        scale.className = 'sc-cov-scale';
        scale.setAttribute('role', 'img');
        var visible = model.binInfo.filter(Boolean);
        scale.setAttribute('aria-label', 'Chip shade shows flight legs per country, from ' +
            (visible[0] ? visible[0].label : '') + ' up to ' +
            (visible.length ? visible[visible.length - 1].label : '') + ' legs.');
        var html = '<span>LEGS PER COUNTRY</span>';
        visible.forEach(function (b) {
            html += '<span class="sc-cov-sw" style="background:' + b.color + ';color:' +
                U.inkOn(b.color) + '">' + b.label + '</span>';
        });
        if (model.unknownCountries) {
            html += '<span class="sc-cov-sw" style="background:' + UNKNOWN_BG +
                ';color:#94a3b8">n/a</span>';
        }
        scale.innerHTML = html;
        fig.appendChild(scale);

        model.continents.forEach(function (a) {
            var unk = isUnknown(a.continent);
            var block = document.createElement('div');
            block.className = 'sc-cov-c';

            var head = document.createElement('div');
            head.className = 'sc-cov-head';
            head.innerHTML =
                '<span class="sc-cov-name"' + (unk ? ' style="color:#94a3b8"' : '') + '>' +
                U.esc(a.continent) + '</span>' +
                '<span class="sc-cov-meta">' + U.plural(a.countries.length, 'country', 'countries') +
                ' · ' + U.plural(a.flights, 'leg') + ' · ' + U.km(a.km) +
                ' · ' + U.pctStr(a.km, model.totalKm) + ' of distance</span>';
            block.appendChild(head);

            var bar = document.createElement('div');
            bar.className = 'sc-cov-bar';
            bar.setAttribute('role', 'img');
            bar.setAttribute('aria-label', a.continent + ': ' + U.km(a.km) + ', ' +
                U.pctStr(a.km, model.totalKm) + ' of all distance flown');
            var inner = document.createElement('i');
            inner.style.width = Math.max(0.8, a.km / model.maxContinentKm * 100) + '%';
            if (unk) inner.style.background = UNKNOWN_BG;
            bar.appendChild(inner);
            block.appendChild(bar);

            var chips = document.createElement('div');
            chips.className = 'sc-cov-chips';
            a.countries.forEach(function (c) {
                var cUnk = isUnknown(c.country);
                var bg = cUnk ? UNKNOWN_BG : model.ramp[Math.min(c.bin, model.ramp.length - 1)];
                var chip = document.createElement('span');
                chip.className = 'sc-cov-chip';
                chip.style.background = bg;
                chip.style.color = cUnk ? '#94a3b8' : U.inkOn(bg);
                chip.innerHTML = U.esc(c.country) + ' <b>' + U.nf(c.flights) + '</b>';
                chip.title = c.country + ' — ' + U.plural(c.flights, 'leg') + ', ' + U.km(c.km);
                U.hover(chip, function () {
                    return '<b>' + U.esc(c.country) + '</b> <i>' + U.esc(c.continent) + '</i>' +
                        '<div class="sc-tip-rows">' +
                        '<span><i>Legs</i></span><span>' + U.nf(c.flights) + '</span>' +
                        '<span><i>Distance</i></span><span>' + U.km(c.km) + '</span>' +
                        '<span><i>Share of continent</i></span><span>' + U.pctStr(c.km, a.km) + '</span>' +
                        '</div>';
                });
        ...
        U.note(fig, 'A leg is counted for both the country it left and the country it landed in, ' +
            'so continent totals add to more than the number of flights.' +
            (model.unknownCountries
                ? ' Countries still awaiting location enrichment are grouped as Unknown.'
                : ''));
    }

    window.StatsCharts.coverage = {
        render: function (container, data, opts) {
            ...
            if (!model.countries.length) {
                ...
                var p = document.createElement('p');
                p.className = 'sc-note';
                p.textContent = 'No country coverage available.';
                container.appendChild(p);
                return;
            }
            U.responsive(container, function (host, W) { draw(host, W, model); });
        }
    };
})();
